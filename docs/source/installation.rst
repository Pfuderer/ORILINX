...
Model discovery: The CLI requires a checkpoint file to be placed in a `models/` directory (searched upward from CWD). The newest `.pt` is used; the command will fail if no checkpoint is found. Note: the DNABERT-2 model folder is also discovered from `ORILINX_DNABERT_PATH` or any child folder of a `models/` directory (searched upward from CWD). If DNABERT is missing you can either place it under `models/`, set `ORILINX_DNABERT_PATH`, add it as a git submodule (e.g., `git submodule add https://huggingface.co/zhihan1996/DNABERT-2-117M models/DNABERT-2-117M` and `git submodule update --init --recursive`), or use the `--fetch_dnabert` flag (or set `ORILINX_DNABERT_AUTO_FETCH=1`) to have ORILINX attempt to download `zhihan1996/DNABERT-2-117M` from Hugging Face at runtime (requires the `huggingface-hub` package).
