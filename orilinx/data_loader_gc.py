...
class OriginClassificationDataset(Dataset):
    ...
    def _load_positive_samples(self, csv_path):
        ...
        with open(csv_path, newline="") as f:
            ...
            for row in reader:
                ...
                new_start = midpoint - (self.sequence_length // 2)
                new_end = midpoint + (self.sequence_length // 2)
                samples.append((row['chromosome'], new_start, new_end))
        return samples
    ...
    def _generate_negative_samples(self, num_negative, positive_samples, seed):
        ...
        # --- HIGHLIGHTED CHANGE: Setup for 50/50 Mixed Negative Sampling ---
        num_gc_matched = num_negative // 2
        num_random = num_negative - num_gc_matched
        negative_samples = []
        attempts = 0
        last_print_time = time.time()
        # --- END HIGHLIGHTED CHANGE ---
        
        # --- HIGHLIGHTED CHANGE: Loop 1 for GC-Matched "Hard" Negatives ---
        print(f"\n[Dataset] Generating {num_gc_matched} GC-matched 'hard' negative samples...")
        while len(negative_samples) < num_gc_matched:
            attempts += 1
            ...
            if current_time - last_print_time > 5 or attempts % 100000 == 0:
                print(f"[Dataset] Progress (GC-matched): {len(negative_samples)}/{num_gc_matched} samples found after {attempts} attempts...")
                last_print_time = current_time
        # --- END HIGHLIGHTED CHANGE ---

        # --- HIGHLIGHTED CHANGE: Loop 2 for Random "Easy" Negatives ---
        print(f"\n[Dataset] Generating {num_random} random 'easy' negative samples...")
        initial_gc_matched_count = len(negative_samples)
        while len(negative_samples) < num_negative:
            ...
            # No GC check for this loop, just append
            negative_samples.append((chrom, start, end))

            current_time = time.time()
            if current_time - last_print_time > 5 or attempts % 100000 == 0:
                print(f"[Dataset] Progress (Random): {len(negative_samples) - initial_gc_matched_count}/{num_random} samples found...")
                last_print_time = current_time
        # --- END HIGHLIGHTED CHANGE ---
        
        print(f"\n[Dataset] Finished generation. Total attempts: {attempts}. Found {len(negative_samples)} total negative samples.")
